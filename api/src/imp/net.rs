...
pub fn sys_connect(fd: i32, addr: UserConstPtr<u8>, addrlen: u32) -> LinuxResult<isize> {
    let addr = to_socketaddr(addr, addrlen)?;
    info!("sys_connect <= fd: {}, addr: {:?}", fd, addr);
...
pub fn sys_getsockname(
    ...
    info!("sys_getsockname <= fd: {}, addr: {:?}", fd, local_addr);
...
pub fn sys_getpeername(
    ...
    info!("sys_getpeername <= fd: {}, addr: {:?}", fd, peer_addr);
...
pub fn sys_accept(fd: i32, addr: UserPtr<u8>, addrlen: UserPtr<socklen_t>) -> LinuxResult<isize> {
    ...
    let fd = socket
        ...
        .map(|fd| fd as isize)
        .map_err(|_| LinuxError::EMFILE)?;
    info!("sys_accept => fd: {}, addr: {:?}", fd, remote_addr);
...
pub fn sys_recvfrom(
    ...
    info!("sys_recvfrom <= fd: {}, len: {}, flags: {}", fd, len, flags);
    let socket = Socket::from_fd(fd)?;
    ...
    info!("sys_recvfrom => fd: {}, recv: {}", fd, recv);
    Ok(recv as isize)
}
